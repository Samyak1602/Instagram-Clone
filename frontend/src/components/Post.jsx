...
const Post = ({ post }) => {
    ...
    const commentHandler = async () => {
        ...
        try {
            const res = await axios.post(`https://localhost:8000/api/v1/post/${post._id}/comment`, { text }, {
                headers: {
    ...
    const deletePostHandler = async () => {
        try {
            const res = await axios.delete(`https://localhost:8000/api/v1/post/delete/${post?._id}`, { withCredentials: true })
            if (res.data.success) {
